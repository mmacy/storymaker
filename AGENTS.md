...
- **Narrated text:** the page sends `narrationText()` to `narrate`, which is the authored segments only by default; the **"Include starter text"** checkbox (default off) prepends the starter. Toggling it changes the cache key (`narration.cacheText`), so the button correctly reverts to "Generate audio" when the audio no longer matches.
