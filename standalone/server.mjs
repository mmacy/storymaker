// Storymaker — standalone local web app.
//
// This server replaces the Copilot CLI extension's native WebView + WebSocket
// bridge with a plain, dependency-light Node server, while keeping the
// extension's *engine* (the Ollama loop + Kokoro narration) and its *page UI*
// (content/, copied here verbatim) completely unchanged. Two browser-native
// primitives stand in for the old bridge:
//   - POST /api/rpc      request/reply      -> the page's window.copilot.<method>()
//   - GET  /api/events   Server-Sent Events -> host pushes that run in the page
//
// Run it with:  node server.mjs   (or  npm start)

import { createServer } from "node:http";
import { readFile, writeFile, mkdir, stat } from "node:fs/promises";
import { extname, join, basename, dirname, normalize, sep } from "node:path";
...
// ===== config ==============================================================
const APP_NAME = "Storymaker"; // shown in the startup banner
const PUSH_NS = "sm"; // the page-side push namespace (window.sm.*)
const CONTENT_DIR = join(__dirname, "content"); // the extension's content/, copied here verbatim
const PORT = Number(process.env.APP_PORT) || 4757;
...
// ---------------------------------------------------------------------------
// SSE push channel. `push(method, payload)` reproduces the extension's
// host->page push: it builds the exact `window.sm.<method>(...)` call the
// extension's own push helper built, and ships it to every connected page to
// run. (Identical semantics to the old `webview.eval(code)`, just over SSE.)
// ---------------------------------------------------------------------------
...
function broadcast(event, data) {
    ...
    for (const res of clients) {
        try { res.write(frame); } catch { /* dropped on its own close handler */ }
    }
}
...
async function push(method, ...args) {
    const code = `window.${PUSH_NS} && window.${PUSH_NS}.${method}(${args.map((a) => JSON.stringify(a)).join(",")})`;
    broadcast("eval", { code });
}

// Stand-in for the extension's session.log(...).
function log(msg) {
    process.stdout.write(`${msg}\n`);
}

// ===== engine ==============================================================
// Lifted verbatim from the extension's main.mjs (the Ollama client, prompt
// construction, generation loop, file saving, and Kokoro narration). The only
// changes: `push(...)` is now the SSE broadcast above, and `session.log(...)`
// became `log(...)`.
...
function saveStatePatch(patch) {
    stateWriteChain = stateWriteChain.then(async () => {
        try {
            ...
            log(`Storymaker: could not persist state (${err?.message || err}).`);
        }
    });
...
// Write text to the OS clipboard from the server (Node) side. We shell out to the
// platform clipboard utility. Linux tries xclip then wl-copy.
...
// server (Node) side. macOS uses `open -R`; Windows uses Explorer's /select;
...
// Read text from the OS clipboard (the inverse of osClipboardCopy).
...
function buildFrontMatter(meta) {
    ...
    // Generation timing (server-measured) for later analysis. Seconds are
...
async function saveStory({ filename, content, meta }) {
    try {
        ...
        log(`Storymaker saved the story to ${target}`);
...
// ---- Kokoro narration ----------------------------------------------------
// Text-to-speech runs entirely server-side (like the Ollama loop): the page
// never touches the model. kokoro-js loads the 82M Kokoro model via
...
// without the page re-sending several MB of base64 back to the server.
...
async function saveAudio({ filename, key } = {}) {
    try {
        ...
        await saveStatePatch({ lastSaveDir: dirname(target) });
        log(`Storymaker saved narration audio to ${target}`);
        return { ok: true, path: target };
    } catch (err) {
        return { ok: false, error: err?.message || String(err) };
    }
...
// ===== callbacks ===========================================================
// The methods the page calls as window.copilot.<method>(...). This is exactly
// the object the extension passed to CopilotWebview({ callbacks }).
const callbacks = {
    listModels: async () => {
        try {
...
// ---------------------------------------------------------------------------
// /__bridge.js — served to the page in place of the extension's bridge. Exposes
// window.copilot over fetch and runs host pushes that arrive over SSE. The
// extension's content/index.html already loads /__bridge.js, so it needs NO
// changes.
// ---------------------------------------------------------------------------
const BRIDGE_JS = `(() => {
  ...
  es.addEventListener("eval", (ev) => {
    try { (0, eval)(JSON.parse(ev.data).code); } catch (e) { console.error("[bridge]", e); }
  });
})();`;

// Served for /favicon.ico so the browser console stays clean (content/ is left
// untouched). The orange wave mirrors the brand mark in the header.
const FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 40 40">` +
    `<path d="M4 26 C 10 8, 16 8, 20 20 S 30 32, 36 14" fill="none" stroke="#F8991C" stroke-width="4" stroke-linecap="round"/></svg>`;
...
async function handleRpc(req, res) {
    ...
    if (typeof fn !== "function") return sendJson(res, 404, { error: `Unknown method: ${method}` });
    try {
...
// ---------------------------------------------------------------------------
// Static serving (the extension's content/, plus the synthetic /__bridge.js)
// ---------------------------------------------------------------------------
const MIME = {
...
async function serveStatic(pathname, res) {
    ...
    const abs = normalize(join(CONTENT_DIR, rel));
    if (abs !== CONTENT_DIR && !abs.startsWith(CONTENT_DIR + sep)) {
...
// A streaming RPC (generateStory / narrate) can hold its response open for
// minutes while pushes stream over SSE — never time it out.
...
function openBrowser(url) {
    ...
    const [cmd, args] =
        ...
        ["xdg-open", [url]];
    try { spawn(cmd, args, { stdio: "ignore", detached: true }).unref(); } catch { /* fine */ }
}
